...
import {
    TbBeach, TbMountain, TbTree, TbSun,
    TbFlower, TbCampfire,
    TbSunset, TbSunrise,
    TbSwimming, TbSailboat,
    TbKayak, TbTent,
    TbCompass, TbMap, TbCamera, TbBackpack,
    TbAnchor,
    TbShip, TbLifebuoy, TbCaravan,
    TbMotorbike, TbTrain, TbBus, TbPlane,
    TbHelicopter, TbRocket, TbBalloon,
    TbBuilding, TbTower,
    TbRoad, TbBusStop,
                ...
                {categories?.map((item, index) => {
                        ...
                        return (
                            <div className="flex" key={item?.id}>
                                <div className="flex flex-col w-32 sm:w-20 pl-2 h-full sm:h-16 border-b-2 border-b-transparent border-solid hover:border-b-2 hover:border-b-black hover:border-solid items-center " onClick={() => { handleClickCategory(item?.id) }}>
                                    {Icon && <Icon className="pt-1" size={30} color="" />}
                                    <h1 className={`${item?.id === activeCategory ? "" : ''} text-sm`}>{item?.name}</h1>
                                </div>
                            </div>
                        )
                    })}
